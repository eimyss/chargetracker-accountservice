podTemplate(label: 'buildpod', containers: [
  ...
  stage('Preparation') { // for display purposes
      ...
      git branch: 'kubernetes', url: 'https://github.com/eimyss/chargetracker-accountservice.git'
   }
        ...
        // TODO: either try sed stuff in another pod (or master), or change kubectl pod ( change docker image) to make writeable folder
        // todo nr: work with variables, and try wiht $WORKSPACE, maybe itll get better. 
            ...
            stage('Kube update deployment') {
                withCredentials([string(credentialsId: 'kube-ca', variable: 'PW1')]) {
                    withKubeConfig(caCertificate: '${PW1}', clusterName: 'kubernetes', contextName: 'jenkins', credentialsId: '37047961-56be-4bf1-a36e-f4651a546fe4', serverUrl: 'https://192.168.123.243:6443') {
                        ...
                        sh 'kubectl --insecure-skip-tls-verify="true" set image deployment.v1.apps/expenses expenses=docker-registry.eimantas.server/expenses-account:1.$BUILD_NUMBER --record=true -n expenses'
